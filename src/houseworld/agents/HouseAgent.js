const Agent = require('../../bdi/Agent');
const pddlActionIntention = require('../../pddl/actions/pddlActionIntention');
const Goal = require('../../bdi/Goal');
const Intention = require('../../bdi/Intention');
const Clock = require('../../utils/Clock');

class FakeAction{
    constructor(agent, parameters){
        this.agent = agent;
        this.parameters = parameters;
    }

    get precondition () {
        return pddlActionIntention.ground(this.constructor.preconditions, this.parameters);
    }
    
    checkPrecondition () {
        return this.agent.beliefs.check(this.precondition);
    }

    get effect () {
        return pddlActionIntention.ground(this.constructor.effect, this.parameters);
    }

    applyEffect () {
        for ( let b of this.effect )
            this.agent.beliefs.apply(b);
    }

    async checkPreconditionAndApplyEffect () {
        if ( this.checkPrecondition() ) {
            this.applyEffect();
            await new Promise(res=>setTimeout(res,this.duration*50));
        }
        else
            throw new Error('pddl precondition not valid');
    }

}

class Move extends FakeAction{
    static parameters = ["robot", "room1", "room2", "base_Station"];
    static preconditions = [["is_in_room", "robot", "room1"],["is_adjacent","room1","room2"]];
    static effect = [["not is_in_room", "robot", "room1"], ["is_in_room", "robot", "room2"], ["not is_in_bs", "base_station", "robot"]];
}

class Clean extends FakeAction{
    static parameters = ["room", "robot"];
    static preconditions = [["is_in_room", "robot", "room"], ["is_dirty","room"]];
    static effect = [["not is_dirty", "room"],["is_clean", "room"]];
}
class Charge extends FakeAction{
    static parameters = ["robot","base_station","room"];
    static preconditions = [["is_in_room", "robot", "room"], ["bs_in_room", "base_station", "room"], ["not is_in_bs","base_station","robot"]];
    static effect = ["is_in_bs", "base_station","robot"];
}

class HouseGoal extends Goal{
    constructor(house){
        super();
        this.house = house;
    }
}

class HouseIntention extends Intention{
    constructor(agent, goal){
        super(agent,goal);
        this.house = goal.house;
        this.Clock = Clock.global;
    }

    static applicable (goal) {
        return goal instanceof HouseGoal;
    }

    *exec(){
        while(true){
            var status = yield this.Clock.notifyChange('hh');
            for (let [key_l, light] of Object.entries(this.house.lights)){
                light.energyConsumption(this.Clock);
            }
            for (let [key_t, therm] of Object.entries(this.house.thermostats)){
                therm.energyConsumption(this.Clock);
            }
            for (let [key_r,room] of Object.entries(this.house.rooms)){
                if (key_r != 'away'){
                    room.update_temperatue();
                }
            }
            if (this.Clock.hh==0){
                console.log('Daily electricy consumption: ' + this.house.total_consumption +' W');
                this.house.total_consumption = 0;
            }
        }
    }
}

class HouseAgent extends Agent{
    constructor (name){
        super(name);
    }

    move({room1 , room2, robot} = args) {
        this.log('move', room1 , room2, robot);
        return new Move(this, {room1 , room2, robot} ).checkPreconditionAndApplyEffect()
        .catch(err=>{this.error('HouseAgent.move failed:', err.message || err); throw err;});
    }

    clean({robot, room} = args) {
        this.log('clean', robot, room,);
        return new Clean(this, {robot, room} ).checkPreconditionAndApplyEffect()
        .catch(err=>{this.error('HouseAgent.clean failed:', err.message || err); throw err;});
    }

    charge({robot, room, base_station} = args) {
        this.log('charge', robot, room, base_station);
        return new Charge(this, {robot, room, base_station} ).checkPreconditionAndApplyEffect()
        .catch(err=>{this.error('HouseAgent.charge failed:', err.message || err); throw err;});
    }
}

module.exports = {HouseAgent,HouseGoal,HouseIntention};